...
public function details($slug)
{
    ...
    return view('Template::user.campaign.details', compact('campaign'));
}
  public function saveCampaign(Request $request, $id = 0)
{
    $pageTitle = 'Create New Campaign';
    $categories = Category::active()->orderBy('id', 'DESC')->get();
    $campaign = $id == 0 ? new DaanCampaign() : DaanCampaign::findOrFail($id);
    $user = auth()->user();
    $isAdmin = $user->firstname == "admin";
    $step = $request->step;
    ...
    if ($step == 1) {
        ...
        // Generate and store slug here
        $title = $request->campaign_title;
        $slug = Str::slug($title);
        $originalSlug = $slug;
        $count = 1;
        while (DaanCampaign::where('slug', $slug)->exists()) {
 ...
 public function edit($slug) // Change $id to $slug
{
    $pageTitle = "Edit Campaign";
    $campaign = Campaign::where('user_id', auth()->id())
                        ->where('slug', $slug) // Retrieve by slug
                        ->firstOrFail();
  ...
  public function campaignUpdation($slug) // Change $id to $slug
{
    $campaign = Campaign::where('user_id', auth()->id())
                        ->where('slug', $slug) // Retrieve by slug
                        ->firstOrFail();
   ...
   public function frontendSeo($slug) // Change $id to $slug
{
