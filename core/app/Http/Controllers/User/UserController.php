...
class UserController extends Controller
{
    public function home()
    {
        $pageTitle = 'Dashboard';
        $user                          = auth()->user();
        $campaign['total_campaign']    = Campaign::where('user_id', $user->id)->count();
        $campaign['pending']           = Campaign::where('user_id', $user->id)->pending()->count();
        $campaign['received_donation'] = Donation::paid()->whereHas('campaign', function ($q) use ($user) {
            $q->where('user_id', $user->id);
        })->sum('donation');

        $campaign['give_donation'] = Donation::where('user_id', $user->id)->paid()->sum('donation');
        ...
        $campaign['current_balance'] = User::where('id', $user->id)->sum('balance');
        $campaign['expired']         = Campaign::where('user_id', $user->id)->where('goal_type', Status::AFTER_DEADLINE)->where('deadline', '<', now())->count();
        ...
        return view('Template::user.dashboard', compact('pageTitle', 'campaign', 'donations', 'withdraws', 'user'));
    }
