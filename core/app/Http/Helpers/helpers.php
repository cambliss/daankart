...
{
    return $image ?   getImage($image) : asset('assets/images/user/cover.png');
}

function timeLeftUntil($futureDateStr)
{
    $futureDate = new DateTime($futureDateStr);
    $now = new DateTime();
    $difference = $now->diff($futureDate);
    $yearsLeft = $difference->y;
    $monthsLeft = $difference->m;
    $daysLeft = $difference->d;
    $hoursLeft = $difference->h;

    $result = "Left ";
    if ($yearsLeft > 0) {
        $result .= "$yearsLeft year(s) ";
    } elseif ($monthsLeft > 0) {
        $result .= "$monthsLeft month(s) ";
    } elseif ($daysLeft > 0) {
        $result .= "$daysLeft day(s) ";
    } elseif ($hoursLeft > 0) {
        $result .= "$hoursLeft hour(s)";
    } else {
        $result .= "0 day";
    }
